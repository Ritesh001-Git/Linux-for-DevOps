...
# RHEL Priority Cheat Sheet

If your goal is **RHEL administration**, prioritize these areas:

| Priority | Area | Commands |
|---|---|---|
| ⭐⭐⭐⭐⭐ | NetworkManager | `nmcli`, `systemctl` |
| ⭐⭐⭐⭐⭐ | Services | `systemctl` |
| ⭐⭐⭐⭐⭐ | Logs | `journalctl` |
| ⭐⭐⭐⭐⭐ | SELinux | `getenforce`, `sestatus`, `semanage`, `restorecon`, `ausearch` |
| ⭐⭐⭐⭐⭐ | Firewall | `firewall-cmd` |
| ⭐⭐⭐⭐⭐ | Packages | `dnf`, `rpm` |
| ⭐⭐⭐⭐ | RHEL subscriptions | `subscription-manager` |
| ⭐⭐⭐⭐ | Networking | `ip`, `ss`, `ping`, `ip neigh` |
| ⭐⭐⭐⭐ | Storage/LVM | `lsblk`, `findmnt`, `pvs`, `vgs`, `lvs` |
| ⭐⭐⭐⭐ | SSH | `sshd`, `journalctl` |
| ⭐⭐⭐⭐ | Time | `chronyc`, `timedatectl` |
| ⭐⭐⭐ | Boot | `systemd-analyze`, `journalctl -b` |
| ⭐⭐⭐ | Containers | `podman` |
| ⭐⭐⭐ | Automation | `ansible` |
| ⭐⭐⭐ | Web administration | `cockpit` |

---

# RHEL Admin Quick Start

For a new RHEL server, these are among the most useful commands to know:
...
For a RHEL administrator, the most important non-basic Linux areas to master are:

**NetworkManager → systemd → journalctl → DNF → repositories/subscriptions → SELinux → firewalld → LVM/XFS → SSH → chrony → Podman → Ansible → Cockpit.**
